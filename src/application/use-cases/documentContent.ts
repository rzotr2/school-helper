...
function isPersistedPage(value: unknown): value is PersistedPageContent {
  if (typeof value !== 'object' || value === null) return false;
...
export function parseDocumentContent(value: unknown): DocumentContent | null {
  ...
  return { pages: content.pages as PersistedPageContent[] };
}
...
export function withPageOcrUpdate(
  ...
  const persistedPage: PersistedPageContent =
 ...
 * by another tab) — the merge in withPageOcrUpdate applies on top of the
...
export async function saveDocumentPage(
  ...
  const merged = withPageOcrUpdate(parseDocumentContent(row.content), page);
